layui.use(['form'], function () {
            ...
            success: function (response, status, xhr) {
                ...
                if (dataCarrier.event !== 'add-item') {
                    form.val('modify-form', {
                        'orgTypeCode': orgVO.orgTypeCode,
                        'orgTypeName': orgVO.orgTypeName,
                ...
                if (dataCarrier.event === 'edit-item') {
                    $('input[name="orgTypeCode"]').prop('disabled', true);
                }
    ...
    form.verify({
        repeated(value, item) { // 重复性校验
            ...
            let count = 0;
            let url = $WEB_ROOT_PATH + '/org-api/org-type';
            $.ajax({
                type: 'get',
                url: url,
                async: false,
    ...
    form.on('submit(submit-modify)', function (data) {
        ...
        let url = $WEB_ROOT_PATH + '/org-api/org-type/modify', type, message;
